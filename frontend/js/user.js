...
function initializeParticipant() {
  ...
  try {
    identity = JSON.parse(sessionStorage.getItem(storageKey) || "null");
  } catch (_) {
    /* storage is optional */
  }
  const show = (name) =>
  ...
  const setConnection = (state) => {
    ...
    [
      ...
      document.querySelector("#quiz-connection"),
    ].forEach((node) => {
      if (node) {
        node.textContent = text;
        node.classList.toggle("reconnecting", state !== "connected");
      }
    });
  };
  ...
  async function loadQuizInfo() {
    try {
      const data = await TechQuizApi.request(
        `/api/quizzes/${encodeURIComponent(quizId)}`,
      );
      document.querySelector("#join-title").textContent = data.name;
      document.querySelector("#join-subtitle").textContent =
        data.status === "WAITING"
          ? "Enter your name to enter the waiting room."
          : "This quiz has already started or finished.";
      if (!identity && data.status !== "WAITING") {
        document.querySelector("#join-button").disabled = true;
        TechQuizApi.setNotice(
          document.querySelector("#join-message"),
          "New participants can only join before the host starts the quiz.",
          "error",
        );
      }
    } catch (error) {
  ...
  function handleEvent(event) {
    ...
    if (event.type === "QUESTION_ENDED") {
      ...
      document.querySelector("#answer-feedback").textContent =
        "Time up! Waiting for the next question…";
      return;
    }
  ...
  function renderState(state) {
    ...
    show("quiz");
    serverOffset = state.server_time
      ? new Date(state.server_time).getTime() - Date.now()
      : serverOffset;
  ...
  function renderQuestion(question) {
    ...
    document.querySelector("#answer-feedback").textContent =
      submittedQuestionId === question.id
        ? "Answer submitted and locked ✓"
        : "Answer quickly—your answer locks after selection.";
    if (submittedQuestionId === question.id) lockOptions();
  }

  function lockOptions(selected) {
    document
      .querySelectorAll("#answer-options .answer-option")
      .forEach((button) => {
        ...
        button.classList.toggle(
          ...
          selected
            ...
            : button.dataset.option ===
                document.querySelector(".answer-option.locked")?.dataset.option,
        );
      });
  }
  ...
  async function sendAnswer(selectedOption, selectedButton) {
    ...
    document.querySelector("#answer-feedback").textContent =
      "Submitting answer…";
    try {
      await TechQuizApi.request(
        `/api/quizzes/${encodeURIComponent(quizId)}/answer`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  function updateTimer() {
    ...
    const seconds = Math.max(
      0,
      Math.ceil(
        (new Date(endTime).getTime() - (Date.now() + serverOffset)) / 1000,
      ),
    );
  ...
  async function showResult() {
    ...
    try {
      const data = await TechQuizApi.request(
        `/api/quizzes/${encodeURIComponent(quizId)}/my-result?participant_token=${encodeURIComponent(identity.participant_token)}`,
      );
      ...
      // Rank is intentionally not shown to participants.
      document.querySelector("#result-name").textContent = "";

      document.querySelector("#my-result-cards").innerHTML = `
        <article class="podium-card">
          <div class="medal">✓</div>
          <div class="podium-name">${data.correct}</div>
          <div class="podium-score">Correct answers</div>
        </article>

        <article class="podium-card">
          <div class="medal">🏆</div>
          <div class="podium-name">${data.score} points</div>
          <div class="podium-score">Total score</div>
        </article>

        <article class="podium-card">
          <div class="medal">⚡</div>
          <div class="podium-name">${data.total_response_time}s</div>
          <div class="podium-score">Response time</div>
        </article>
      `;

      // Participant ranking table is intentionally not rendered.
      const resultTable = document.querySelector("#my-result-table");
      if (resultTable) {
        resultTable.innerHTML = "";
      }
    } catch (error) {
      document.querySelector("#result-name").textContent = error.message;
    }
  }
  ...
  document
    ...
    .addEventListener("submit", async (event) => {
      ...
      if (!name)
        return TechQuizApi.setNotice(message, "Name is required.", "error");
  ...
  if (identity?.participant_token) {
    document.querySelector("#waiting-name").textContent =
